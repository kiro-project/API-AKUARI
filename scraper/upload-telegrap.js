...
const uploadByUrlanon = (url, agent) => {
  return fetch(url)
    .then(async (r) => {
      if (!isStream(r.body)) {
        throw new TypeError('Response is not a stream')
      ...
      return uploadanon(buffer, r.headers.get('content-type'), agent)
    })
}


const uploadanon = (buffer, contentType, agent) => {
  if (!isBuffer(buffer)) {
    throw new TypeError('Buffer is not a Buffer')
  }
  const form = new FormData()
  ...
  return fetch('https://api.anonfiles.com/upload', {
    method: 'POST',
    body: form
  })
    .then(result => result.json())
